...
# ==============================================================================
    ...
    # 2. CASCO 2D & 3D
        ...
        def get_waterlines_figure():
            ...
            # Cortes Longitudinais A, B, C como linhas horizontais de referência
            cuts_y_ref = [0.34, 0.68, 1.02]
            for cy, c_lbl in zip(cuts_y_ref, ["A", "B", "C"]):
                fig.add_hline(y=cy, line_dash="dot", line_color="rgba(148, 163, 184, 0.25)", line_width=1.0)
            ...
            fig.update_layout(
                title="Plano de Linhas d'Água (Half-Breadth Plan / Vista Superior Oficial - Conforme PDF)",
                xaxis_title="Comprimento Longitudinal X (m) [ST 00 (Popa) → ST 10 (Proa)]",
                yaxis_title="Meia-boca Y (m) [Linha de Centro LC = 0 → Borda]",
        ...
        def get_sheer_figure():
            ...
            z_stem_eval = np.clip(pchip_stem(xs_eval), 0.0, hull.D)
            
            fig.add_trace(go.Scatter(
                x=xs_eval, y=z_stem_eval, mode='lines',
            ...
            # 4. Linhas do Alto (Cortes A, B, C a distâncias Y constantes da LC)
            # Cortes oficiais: 340mm (Corte A), 680mm (Corte B), 1000mm (Corte C)
            cuts_data = [
                {"y": 0.340, "name": "Corte A (Y = 340 mm)", "color": "#c084fc", "w": 2.4},
                {"y": 0.680, "name": "Corte B (Y = 680 mm)", "color": "#f43f5e", "w": 2.4},
            ...
            fig.update_layout(
                title="Plano de Linhas do Alto / Perfil (Sheer / Buttock Plan - Vista Lateral Oficial)",
                xaxis_title="Comprimento Longitudinal X (m) [ST 00 (Popa) → ST 10 (Proa)]",
